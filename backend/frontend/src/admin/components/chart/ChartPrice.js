...
const ChartPrice = (props) => {
    ...
    if (orders && orders.Orders) {
        if (optionPrice === 0) {
            ...
            var temp = orders.Orders.filter(
                (value) => Number(value.createdAt.substring(0, 4)) === year
            );
            let i = min;
            ...
            var temp2 = orders.Orders.filter(
                (value) =>
                    Number(value.createdAt.substring(0, 4)) === year &&
                    Number(value.createdAt.substring(5, 7)) === month
            );
