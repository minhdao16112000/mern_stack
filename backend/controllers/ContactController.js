...
class ContactController {
    ...
    // [GET] /
    show(req, res, next) {
        Promise.all([contactModel.find({}), contactModel.countDocumentsDeleted()])
        ...
        content +=
            ...
            <div>
                ...
                <p style="font-size: 17px; margin-top: 20px;">Nếu có thắc mắc gì khác, bạn vui lòng cho Fashi shop biết tại
                    <a href="http://localhost:3000/contact"><b>đây</b></a>. Chúng tôi rất hân hạnh được phục vụ bạn</p>
            </div>
    ...
    forceDestroy(req, res, next) {
        const ids = req.body.id;
        const idArr = ids.split(',');
        contactModel.deleteOne({ _id: idArr })
    ...
    restore(req, res, next) {
        try {
            ...
            ids.forEach(value => contactModel.restore({ _id: value }, (err, result) => {
                if (err) throw err;
                console.log(result);
            }));
    ...
    active = async (req, res, next) => {
        try {
            ...
            contactModel.findOneAndUpdate({ _id: contact.id }, seen, {
