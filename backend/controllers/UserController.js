...
class UserController {
    ...
    changePassword = async (req, res, err) => {
        ...
        userModel.findOneAndUpdate({ _id: newUser.id }, newUser, {
            returnOriginal: false
        })
            .then((User) => res.send({ message: 'Change Password Successfully', user: User }))
    ...
    forceDestroy(req, res, next) {
        const ids = req.body.id;
        const idArr = ids.split(',');
        userModel.deleteMany({ _id: idArr })
    ...
    restore(req, res, next) {
        try {
            ...
            ids.forEach(value => userModel.restore({ _id: value }, (err, result) => {
                if (err) throw err;
                console.log(result);
            }));
