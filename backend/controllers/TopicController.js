...
class TopicController {
    ...
    // [PUT] /:id
    update(req, res, next) {
        topicModel.updateOne({ _id: req.params.id }, req.body)
    ...
    restore(req, res, next) {
        try {
            ...
            ids.forEach(value => topicModel.restore({ _id: value }, (err, result) => {
                if (err) throw err;
                console.log(result);
            }));
    ...
    active = async (req, res, next) => {
        try {
            ...
            topic.status === "1"
                ? topicModel.findOneAndUpdate({ _id: topic.id }, hidden, {
                    returnOriginal: false
                })
                    .then(() => res.send('hidden'))
                    .catch(next)
                : topicModel.findOneAndUpdate({ _id: req.params.id }, show, {
                    returnOriginal: false
                })
                    .then(() => res.send('show'))
                    .catch(next)
    ...
    forceDestroy(req, res, next) {
        ...
        topicModel.deleteMany({ _id: idArr })
